...
`cm/as-is/mgt/gcc/mgt.xml`(`$c.mgt`)에 적용한 함수 수정 사항을 정리한다.
...
- API 문서: `cm/docs/api/mgt/index_trk.html` 신규 생성, `index_mgt.html`에서 trk 제거.
...
- API 문서: `cm/docs/api/mgt/index_print.html` 신규 생성, `index_mgt.html`에서 print 함수 제거.
- `python -m wsxml_lint cm/as-is/mgt/gcc/print.xml`이 `0 errors, 0 warnings`인지 확인.
...
검증: `python -m wsxml_lint cm/as-is/mgt/gcc/mgt.xml` → `0 errors, 0 warnings`.
