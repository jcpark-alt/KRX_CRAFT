...
> `cm/as-is/mgt` 의 모든 화면 함수(`scwin.*`)를 추출하여 이관/삭제 여부를 분류한 마스터 문서.
> TO-BE 공통 위치: `cm/gcc`. **본 문서는 분석 가이드이며 소스는 수정하지 않는다.**

## 집계 요약

- **분석 대상:** `cm/as-is/mgt` 18개 `.xml` 파일, **196개** `scwin.*` 함수(기계적 인벤토리 기준).
