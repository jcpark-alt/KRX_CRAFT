...
`cm/as-is/stf/gcc/stf.xml`(`$c.stf`)에 추출·적용한 함수 내역을 정리한다.
원본 `cm/as-is/stf/stf.xml`(71개 `scwin.*`)에서 **업무공통(KRX 고유) 코어** 함수만 골라
...
`report.xml`(AS-IS 소비 네임스페이스 `$c.rpt.*`)의 **리포트 엔진 16개 함수**를 `cm/as-is/stf/gcc/print.xml`(`$c.print`)로 분리·생성했다(camelCase 표준화 + AS-IS 병기, lint 0/0).
...
> 이후 `cm/docs/api/stf/index_print.html` API 문서 생성 권장.

---

검증: `python -m wsxml_lint cm/as-is/stf/gcc/stf.xml cm/as-is/stf/gcc/print.xml` → `0 errors, 0 warnings`.
