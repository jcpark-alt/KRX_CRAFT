...
> `cm/as-is/stf` 의 모든 화면 함수(`scwin.*`)를 추출하여 이관/삭제 여부를 분류한 마스터 문서.
> TO-BE 공통 위치: `cm/gcc`(공통이관) · `cm/as-is/stf/gcc/stf.xml`(업무공통). **본 문서는 분석 가이드이며 원본 소스는 수정하지 않는다.**

## 집계 요약

- **분석 대상:** `cm/as-is/stf` **48개** `.xml` 파일, **962개** `scwin.*` 함수 정의(기계적 인벤토리 기준 `scwin.X = function`).
...
stf 모듈의 **중심 업무공통 파일**. 단순 문자열/숫자/날짜/검증 함수는 기존 gcc 로 매핑하고, KRX 고유 업무공통 코어 함수만 `cm/as-is/stf/gcc/stf.xml`(`$c.stf`)로 추출했다.
...
인쇄/리포트 전용 함수는 `stf.xml` 에 두지 않고 별도 `print.xml`($c.print)로 분리한다(분리 규칙은 `cm/as-is/stf/gcc/md/README.md`).
**`report.xml`(AS-IS `$c.rpt.*`)의 리포트 엔진 16개 함수**를 `cm/as-is/stf/gcc/print.xml`(`$c.print`)로 이관 완료(camelCase 표준화, lint 0/0).
