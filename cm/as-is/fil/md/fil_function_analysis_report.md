...
> `cm/as-is/fil`(root/bnf/inf) 의 모든 화면 함수(`scwin.*`)를 추출하여 이관/삭제 여부를 분류한 마스터 문서.
> TO-BE 공통 위치: `cm/gcc`(공통이관) · `cm/as-is/fil/gcc`(업무공통 `$c.fil`/`$c.trk`/`$c.print`) · `$c.ins`(ins 동일 사본 재사용). **본 문서는 분석 가이드이며 원본 소스는 수정하지 않는다.**

## 집계 요약

- **분석 대상:** `cm/as-is/fil` 111개 `.xml` (root 45 + `bnf` 30 + `inf` 36), **약 1,316개** `scwin.* = function` 정의(기계적 인벤토리 기준 — minified jQuery/서드파티 라이브러리 본체 포함).
