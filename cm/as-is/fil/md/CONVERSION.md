...
> 본 문서/분석 자료는 `cm/as-is/fil/md/` 로 일원화되었다. (분석 결과물 = `cm/as-is/fil/md`, 생성 산출물 소스 = `cm/as-is/fil/gcc`, API HTML = `cm/docs/api/fil`)
> 방식은 `cm/as-is/mgt`(`$c.mgt`/`$c.trk`)·`ins`(`$c.ins`/`$c.trk`/`$c.print`)에서 검증된 절차를 동일하게 따른다.
...
1. **분석 마스터 리포트 파일:** `cm/as-is/fil/md/fil_function_analysis_report.md`
   - *역할:* `fil` 폴더(root/bnf/inf)에서 추출된 전체 함수 목록의 필터링 결과 및 [삭제 대상/이관 대상] 분류 이력 확인.
2. **이관 및 매핑 가이드 파일:** `cm/as-is/fil/md/gcc_mapping_and_biz_common.md`
...
3. **[Plan-3] 소스 및 문서 빌드:** 업무공통 전용 `cm/as-is/fil/gcc/fil.xml`(`$c.fil`) 소스와 독립형 API 문서 `cm/docs/api/fil/index_fil.html` 을 생성한다.
...
## 4. 신규 업무공통함수 파일 (`cm/as-is/fil/gcc/fil.xml`) 생성 요구사항
기존 `gcc` 공통함수에 매핑되지 않고 '업무공통'으로 분류된 함수들은 `cm/as-is/fil/gcc/fil.xml`(`$c.fil`) 로 빌드한다.
- **XML 구조 규격:** WebSquare5 공통 스크립트 표준 구조(html/xmlns/head/`xf:model`/`w2:publicInfo`/script CDATA)를 그대로 따른다.
- **publicInfo 동기화·lint:** 공개 함수는 `<w2:publicInfo>` 에 등재하고 `python -m wsxml_lint cm/as-is/fil/gcc` 이 **0 errors/0 warnings** 여야 한다.
...
| 항목 | 상태 |
| :--- | :--- |
| 분석 문서 2종 (`fil_function_analysis_report.md`, `gcc_mapping_and_biz_common.md`) | `cm/as-is/fil/md/` 작성 완료 |
| 업무공통 7개 함수 → `cm/as-is/fil/gcc/fil.xml`(`$c.fil`) | 생성 완료 — **camelCase 표준화 + AS-IS 병기**, lint 0/0 |
| 분석/트래킹 8개 함수 → `cm/as-is/fil/gcc/trk.xml`(`$c.trk`) | 분리 완료 (origin: logger_tracking.xml, ins/stf `$c.trk` 동일), lint 0/0 |
| 문서 인쇄 1개 함수 → `cm/as-is/fil/gcc/print.xml`(`$c.print`) | 분리 완료 (origin: report.xml, MarkAny 포함), lint 0/0 |
...
| lint 범위 편입 | `package.json`/`ci.yml`/`CLAUDE.md` 의 `lint:xml:legacy` 에 `cm/as-is/fil` 추가(jQuery orphan publicInfo·dtree 인코딩 정리로 0 warnings) |

> 원본 `cm/as-is/fil/*.xml`(및 `bnf`/`inf`)의 함수는 보존 상태이며, 호출부를 TO-BE(`$c.*`)로 전환한 뒤 제거하는 것을 권장한다.
