...
> `cm/as-is/ins` 의 모든 화면 함수(`scwin.*`)를 추출하여 이관/삭제 여부를 분류한 마스터 문서.
> TO-BE 공통 위치: `cm/gcc`(공통이관) · `cm/as-is/ins/gcc`(업무공통 `$c.ins`/`$c.trk`/`$c.print`). **본 문서는 분석 가이드이며 원본 소스는 수정하지 않는다.**

## 집계 요약

- **분석 대상:** `cm/as-is/ins` 39개 `.xml` 파일, **약 780개** `scwin.*` 함수(기계적 인벤토리 기준).
