...
└──────────────────────────────────────────────────────────────────────────┘
  ...
  Gauce/X-Internet   →   W-Craft 변환   →   Python 기계 치환   →   Claude Code 보강   →   운영 반영
   레거시 화면             (외부 도구)         (결정적/일괄)          (판단/재설계)         WebSquare 화면
        ...
        │                     │                    │                       │                   │
        ▼                     ▼                    ▼                       ▼                   ▼
   구 플랫폼 소스        ui/*.xml            ui-tobe/*.xml           ui-tobe/*.xml        cm/as-is/**
...
└── next-krx-lds-{fil,mgt,stf,tms}-front/   ← 모듈별 변환 작업 공간
    ...
    └── ui-tobe/     ← 출력: 단계 1 기계 치환본 → 단계 2 보강본
         ...
         [ 단일 출처(SOT) — 도구가 참조 ]
         ├── cm/docs/api/{fil,ins,mgt}/index_transfer.html  → gcc_mapping.substitution_dict()
         ├── cm/as-is/{fil,ins,mgt,stf}/gcc/*.xml (JSDoc)    → gcc_mapping.module_fn_dict()
         └── cm/docs/api/gcc/index.html                       → 치환 함수 시그니처 확인(단계 2)
```
...
```powershell
# 단계 1 — 단일 파일 변환
python cm/conversion/tools/convert.py <src.xml> [out.xml]

# 단계 1 — 모듈 일괄 변환 (fil / mgt / stf / tms, 인자 생략 시 전체)
python cm/conversion/tools/convert_all.py [module]

# 단계 1 — 규칙 개정 후 전체 재생성 (기존 산출물 덮어씀 — 수기 보강 유무 사전 확인)
python cm/conversion/tools/convert_all.py --force [module]

# 단계 2 — 리포트/TODO 보강 후 검증
npm run lint:xml        # cm/gcc 엄격 + cm/as-is 레거시(WS111~113 무시) 모두 0 경고 유지
```
