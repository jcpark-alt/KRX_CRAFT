...
프로젝트별 폴더를 `cm/conversion/` 아래 표준 규격으로 만든다.

```
cm/conversion/<프로젝트명>-front/
...
python cm/conversion/tools/convert_all.py <프로젝트명>-front

# 규칙 개정 후 전체 재생성 (기존 산출물 덮어씀)
python cm/conversion/tools/convert_all.py --force <프로젝트명>-front
```
...
- [ ] `python -m pytest cm/conversion/tools/` — 변환기 회귀 테스트 통과 (규칙 개정을 했다면 필수)
