...
`cm/docs/api/{fil,ins,mgt}/index_transfer.html` 의 `DATA` 배열을 namespace 기준으로 통합한 대표 매핑입니다. (AS-IS = 레거시 함수 / TO-BE = gcc 표준 `$c.*`) 동일 의미의 사본 함수는 슬래시(`/`)로 묶었습니다.

> **프로그램 접근**: 이 표는 사람이 읽는 요약이고, 변환기는 `cm/conversion/tools/gcc_mapping.py` 로더로 위 `DATA`(SOT)를 직접 파싱해 사용합니다. `substitution_dict()` 는 **태그 없는 순수 식별자·무충돌** 항목만 담은 자동 1:1 치환 사전(규칙 7)을, `conflicts()` 는 동일 이름이 다른 `$c.*` 로 갈리는 충돌 항목을 돌려줍니다. (`python cm/conversion/tools/gcc_mapping.py` 로 요약 확인)
...
| `$c.win.moveUrl` | `goURL` | URL 이동 — 목록↔상세 왕복 화면은 `{isHistory:true, dataInfo}`(이동 시 스냅샷)·상세 [목록] 버튼 `{restoreData:true}` 옵션으로 상태 복원 적용 검토(단계 2, `cm/docs/frame-history-guide.md`) | |
...
| `$c.win.getOpenerScope` / `$c.win.callOpener` | `window.opener.*` / browserPopup 화면의 부모 scwin 호출 | 팝업에서 부모 화면 접근 — browserPopup(별도 창)은 `getParent()` 불가. `getOpenerScope()`(부모 scope 반환)·`callOpener(fnName, ...args)`(부모 scwin 함수 호출)는 pageFramePopup 에서도 동일 동작(타입 분기 불요). 상세 `cm/docs/popup-opener-guide.md` | 검토 |
...
이 표는 위 1~8장(레거시 유틸 → `$c.<core>` 대표 치환)과 달리, **모듈별 업무공통(KRX 고유) 함수의 AS-IS 레거시 원본 → 변환된 `$c.<모듈>` 함수 1:1 매핑**이다. `cm/as-is/{fil,ins,mgt,stf}/gcc/*.xml` 각 함수 JSDoc 의 `(AS-IS: 원본명, origin: 원본.xml)` 주석을 취합한 것으로, 변환 추적성(traceability) 규칙은 [stf/md/CONVERSION.md](../../as-is/stf/md/CONVERSION.md) 를 따른다.
...
| `$c.data.downloadGridViewExcel(grid, fileName, sheetName, type)`(위치인자 4개) | `$c.data.downloadGridViewExcel(grid, {fileName, [sheetName,] type})` | convert.py 규칙 20b | 2번째→fileName, 4번째→type, 3번째(빈값) 생략. 인자 수 4개로 형태 판별(객체형 2~3인자 제외), 토큰 원형 보존, 멱등 |
