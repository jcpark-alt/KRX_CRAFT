...
> conversion 트리(`cm/conversion/next-krx-lds-{mgt,stf,tms}-front/ui-tobe/`)의 그리드 엑셀 다운로드 호출을 gcc 공통함수 `$c.data.downloadGridViewExcel` 의 객체 시그니처로 통일한 결과 보고서. 규칙 정의는 [conversion_rules.md](conversion_rules.md) §규칙 20·20b, 매핑표는 [substitution_map.md](substitution_map.md) §10 참조.
...
- 매핑 근거: gcc `cm/gcc/data.xml` 의 `downloadGridViewExcel` JSDoc 시그니처(`grdObj, options{}, infoArr[]`)와 손작성 객체형 선례(`ULDTMS04010`: `option.fileName`/`option.type`).
