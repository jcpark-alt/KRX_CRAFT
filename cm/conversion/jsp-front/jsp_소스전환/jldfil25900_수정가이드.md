...
> 대상: `cm/conversion/jsp-front/jsp_소스전환/jldfil25900.xml` (2구역 초기화 영역)
...
JSP form 제출 기계부를 gcc 공통함수 `$c.win.openFormSubmit(url, params)` (동적 hidden 폼 생성 후 POST 제출 — JSP 페이지 전환 의미 보존, cm/gcc/win.xml) 기반으로 재설계. **body 실측 결과 dividendDateForm(xf:group)에 파일 컨트롤이 없어 multipart 예외 없음 → 제출 2건 전면 전환**, 규칙 19 보류 2건 전량 해소.
...
> `convert.py` 단계1을 제자리 실행하고 후처리로 보정(init 2구역 복원·5b DOM 수신 원복·개명 충돌 조정·JSDoc 동기화). 경위·공통 게이트: [krx_소스전환_미적용분석.md §5 이력 6](krx_소스전환_미적용분석.md). diff +6/−13.

| 항목 | 건수 | 내용 |
|------|------|------|
| 규칙 13 `fn_*` 개명 | 1건 | `fn_modifiyDate`→`selectModifiyDate` — `fn_modifiyDate`→`modifiyDate` 는 상태 변수 `scwin.modifiyDate` 를 덮어쓰므로 **`selectModifiyDate`** 로 조정(정의·호출·JSDoc·publicInfo). 정의·호출·body `ev:`·publicInfo 동기화(도구), JSDoc `@name`/`@example` 옛 이름 2건 후처리 동기화. 교차 화면 호출 없음 |
| 규칙 5a 엄격 비교 | 1건 | `==`/`!=` → `===`/`!==` |
| 규칙 2 전역 선언 이동 | 3건 | 최상위 `scwin.X = …` 선언을 1구역으로 |
| 규칙 4 구역 재배치 | 0건 이동 | `init_*` 4건은 도구가 5구역으로 옮긴 것을 code-convention 대로 2구역(`onpageload` 아래)에 복원 |
...
| `$c.data.readValue(dc, key, opts)` | 2 | dataMap → `dc.get("key")` 2건, dataList → `dc.getCellData(row, "key")` 0건, 변수 키 래퍼 → `.get(key)` 0건. `silent`/`soft`/`ctx`/`label` 옵션은 의미 없어 제거 |
