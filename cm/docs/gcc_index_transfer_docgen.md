...
`cm/docs/api/gcc/index_transfer.html`(레거시 AS-IS → gcc 표준 TO-BE `$c.*` 함수 이관 매핑 통합 가이드)의 **생성 규칙**이다.
...
| 섹션 | SOT | 방식 |
| --- | --- | --- |
| **RAW**(파일별 AS-IS→TO-BE 매핑) | `cm/docs/api/{fil,ins,mgt}/index_transfer.html` 의 `const DATA = [...]` 배열 | `gcc_mapping.load_mappings()` 로 3개 모듈을 읽어 **통합 → `(file, asis, tobe, desc, tag)` 기준 중복제거**. gcc 고유 항목은 없음(전량 모듈 파생). |
...
- `cm/docs/api/{fil,ins,mgt}/index_transfer.html` 의 `DATA` 를 추가/수정했을 때.
