...
`cm/docs/api/gcc/index.html`(gcc 공통 라이브러리 API 문서)의 **생성 규칙**을 정리한 문서다.
...
```bash
npm run docs:gcc
# → python -m wsxml_lint.docgen cm/gcc -o cm/docs/api/gcc --title "gcc API 문서"
```

- 입력: `cm/gcc`의 모든 `.xml` 페이지.
- 출력: `cm/docs/api/gcc/index.html` **단일 파일** (CSS/JS 인라인, 외부 의존성 없음 → `file://` 더블클릭으로 열림).
