...
`cm/gcc`(및 `cm/as-is/mgt/gcc` 등) 의 **WebSquare 공통함수 XML** 파일을 새로 만들거나 함수를 추가할 때 지켜야 할 규칙과 검증 절차를 정리한다.
...
| 항목 | 규칙 |
| :--- | :--- |
| 위치 | 표준 공통: `cm/gcc/<name>.xml` · 모듈 업무공통: `src/<module>/gcc/<name>.xml` |
...
| 목적 | 명령 |
| :--- | :--- |
| 단일 파일 검증 | `python -m wsxml_lint cm/gcc/<name>.xml` |
| gcc 전체(엄격) | `npm run lint:xml:gcc` (= `wsxml_lint cm/gcc`) |
| 레거시(완화) | `npm run lint:xml:legacy` (`cm/as-is/ins cm/as-is/mgt cm/as-is/stf cm/as-is/fil --ignore WS111,WS112,WS113`) |
| 전체 | `npm run lint:xml` |
| API 문서 재생성 | `npm run docs:gcc` → `cm/docs/api/gcc/index.html` |
...
- **분류 후 문서 갱신**: 이관 결과를 `cm/docs/api/mgt/*.md`(분석 리포트·매핑 명세)에 반영(업무공통→공통이관 등).
