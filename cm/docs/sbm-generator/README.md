...
- **디자인**: `cm/docs/DESIGN.md` 디자인 시스템(색상 토큰·2분할 레이아웃·Toast·Copy) 준수.
...
- **근거**: 변환 규칙은 `cm/gcc/sbm.xml` 의 `scwin._normalizeRefTarget` / `scwin._bindResponseToTargets` / `scwin.executeDynamic` JSDoc 주석을 그대로 반영한다.
...
- 변환 규칙 원본: `cm/gcc/sbm.xml` — `scwin.executeDynamic`, `scwin._normalizeRefTarget`, `scwin._bindResponseToTargets`.
- 디자인 가이드: `cm/docs/DESIGN.md`.
- 자매 도구: `cm/docs/data-generator/` (DataCollection XML 생성기).
