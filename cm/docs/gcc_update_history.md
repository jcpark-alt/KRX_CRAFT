...
- `bcd1d06` (09-15) — **`cert.xml`(`$c.cert`) 신설 + `$c.util` DOM 로더 3종** (292→301 메서드, 12→13 모듈):
  ...
  - **실환경 오류 수정 2(09-16)**: 인증서 팝업에서 `ReferenceError: cwGetModuleInstallStatus is not defined`(3rd_interface.js → cwui initializeController) — 정의처 `crosswebexInit.js` 는 crosswebex6.js 가 `document.write` 로 끌어오는데 엔진이 engine 모듈을 비동기 외부 스크립트로 로드해 브라우저가 이를 무시. `cm/cert/js/common/initech-docwrite-capture.js`(crosswebex6 앞, document.write 가로채 `<script src>` 큐잉) + `initech-docwrite-replay.js`(crosswebex6 뒤, 동기 XHR + 인라인 script 로 기록 순서 실행, 중첩 document.write 인 exinterface→exproto_ext_daemon 은 파서 의미대로 현재 스크립트 직후에 삽입, 완료 후 원복·캡처 해제) 로 벤더 조건부 로직을 그대로 보존. transkey.js 도 SHA2 분기에서 document.write 로 jsbn 을 로드하므로 라온 구간에도 같은 쌍을 둔다(config: promise-guard → [capture→transkey→replay] → [capture→crosswebex6→replay] → initech-common). 회귀 `test/initechDocWrite.test.js` 5건
  - **실환경 오류 수정(09-16)**: config.xml 정적 로드 후 메인 레이아웃에서 `TypeError: Promise.allSettled is not a function`(엔진 `_loadWebSquareCommon`) — crosswebex6.js 가 끌어오는 bluebird-3.5.0 이 `window.Promise` 를 덮어쓰기 때문. 이니텍 코드는 `new Promise`/`Promise.all`/`Promise.resolve` 만 쓰므로 `cm/cert/js/common/promise-guard.js`(전역 Promise 를 접근자로 네이티브 고정, 덮어쓰기 대입 무시)를 engine 모듈 맨 앞에 등록. 재현·회귀 `test/promiseGuard.test.js` 4건
