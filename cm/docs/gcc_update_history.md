...
- `bcd1d06` (09-15) — **`cert.xml`(`$c.cert`) 신설 + `$c.util` DOM 로더 3종** (292→301 메서드, 12→13 모듈):
  ...
  - **`$c.cert.openManager` 추가(09-16, 미커밋)**: 이니텍 인증서 관리 화면(HTML5 UI) 호출 — `INIWEBEX.openCertManager({ isHtml5, langType, taskNm, processCallback })` 래핑, `useTranskey` 반영·미로드 시 오류, `popupOptions` 로 벤더 옵션 재정의. SMPCRT10000 에 [인증서 관리] 버튼(`btn_certManage`) 추가. Jest 2건 (301→302 메서드)
