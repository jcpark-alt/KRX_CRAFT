...
- `bcd1d06` (09-15) — **`cert.xml`(`$c.cert`) 신설 + `$c.util` DOM 로더 3종** (292→301 메서드, 12→13 모듈):
  ...
  - **`$c.date` 포맷 오류 시 빈값 초기화 옵션(09-16, 미커밋)**: `checkCalendarFormat(cal, fmt, objName, options)`·`compareFromToDate(start, end, msgOptions, fmt, options)` 에 `options.clearOnInvalid`(기본 **true**) 추가 — onviewchange 포맷 검증 실패 시 안내 후 `setValue("")`. 빈값은 형식 오류로 보지 않도록 해 초기화 재진입 시 안내 반복을 방지(필수 여부는 validateDataCollect 몫). false 면 종전 동작(값 유지·포커스). 기존 호출부(2~4 인자)는 그대로 호환. 회귀 `test/dateClearOnInvalid.test.js` 8건
