...
> 분석 방법: ① 전 파일 규약 패턴 정적 스캔(CDATA 추출 후 정규식 카운트) ② 수작업 최종본이 있는 `jldfil25900`·`25910`(jsp-front 루트)과의 diff 대조(ground truth)
...
## 2. 미적용 항목 — 자동 프로세스 갭 (전 파일 공통)

### 2.1 표준 JSDoc 미적용 — **591/591 함수 전부 placeholder** `⚠ 최다 갭`
...
## 3. 재설계 판단 대상 (자동 치환 불가 — 단계 2 보류 유형)
...
krx_소스전환 사본과 jsp-front 루트 최종본의 차이(각 303줄·1,013줄 변경)가 곧 **자동 프로세스가 못 채운 부분**의 정답지다:
...
| 구분 | 파일 | 비고 |
|------|------|------|
| **정비 완료** (2) | `jldfil25900` · `jldfil25910` | jsp-front 루트에 최종본 — krx_소스전환 사본은 자동 산출물 스냅샷 |
...
| **후속 정비 미착수** (23) | `35700c`(228KB·함수 194) · `35701~35708`(35705 없음, 7파일) · `35706c` · `35710~35717` · `52000` · `52120` · `inf20000p` · `inf90009` · layer 2파일 | §2 갭 전체 해당 |

주요 규모: `jldfil35700c`(228KB, 함수 194, jQuery 46, var 25) > `jldfil59410`(162KB, 함수 85) > `jldinf20000`(87KB, 함수 57) — 3파일이 전체 함수의 57%.

---

## 6. 권고 후속 순서

1. **기계 적용 가능(전 파일 일괄, 자동화 후보)**: publicInfo 등재(+`ipt_method` 오기 교정) → let→const(재할당 분석) → var 제거 → `__` 접두 지역변수 개명 → 과밀 라인 뷰티파이. conversion 파이프라인(editor-web generate)에 규칙 반영 시 재생성만으로 해소 가능.
2. **반자동(템플릿 + 내용 작성)**: JSDoc 591건 — 스텁을 표준 블록으로 전환하되 `@description`/`@param` 설명은 판단 작업.
3. **판단/재설계(단계 2)**: §3 유형 — jQuery 94건(최대 갭: `35700c`), form 제출 12건, `fn_*` 개명 77건, layer 2파일 편입 방식.
4. 순서 제안: 소형 다수(35701~35717, 각 3~7함수)로 패턴 확립 → 중형(52000·52120·59400·inf90009·inf20000p) → 대형 3파일(35700c·59410·inf20000).
