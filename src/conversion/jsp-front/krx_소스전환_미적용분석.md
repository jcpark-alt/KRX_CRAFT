...
> **해소 내역(`a4b49ab`)**: 29화면(25900·25910 제외 전량)을 jsp-front 루트 정비본으로 변환 — 기계 패스(publicInfo 등재·JSDoc 스텁 정리 스크립트) + 판단 패스(9배치 병렬: @description 540/540 작성, let→const, `__` 개명, 과밀 분해, layer 구역 헤더). 화면별 `_수정가이드.md` 29건 동봉, 전수 node --check·wsxml_lint 0 errors. 아래 각 절의 수치는 **정비 전 실측 기록**으로 보존한다.
...
| 구분 | 파일 | 비고 |
|------|------|------|
| **정비 완료 — 선행 정답지** (2) | `jldfil25900` · `jldfil25910` | jsp-front 루트 최종본(`6b32da3`~`7ec2732`) — 이번 일괄 정비의 규약 기준 |
| **정비 완료 — 일괄 배치** (29) | `35700c`(함수 194) · `59410`(87) · `inf20000`(57) · `52100`(36) · `35706c`(24) · `inf20000p`(23) · `52000`(20) · `59400`(20) · 소형 20파일(35700~35717·52120·inf90009·layer 2) | `a4b49ab` — jsp-front 루트 정비본 + 화면별 `_수정가이드.md` 29건. 원본 유래 결함(미해결 참조·죽은 분기 등)은 각 가이드 §3 후속 과제로 기록 |
