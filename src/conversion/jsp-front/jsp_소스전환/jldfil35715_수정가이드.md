# jldfil35715 수정 가이드 — editor-web generate 산출물 후속 정비

> 원본: `../krx_소스전환/jldfil35715.xml` → 정비본: `jsp-front/jldfil35715.xml` (2026-09-07)
...
| 과밀 라인 분해 | 1건 | `init_conds` 의 binds 정의 약 330자 한 줄을 바인딩(2개)별 다행으로 분해. 동일 인자 `$c.data.readValue("dma_pageContext", "kk", …)` 2회 반복 호출을 `const kk` 1회 캐싱으로 치환(진입점 1회 동기 평가라 동작 동등) |
| onpageload 재배치 | 예 | 2구역 최상단으로 이동(코드 동일), `init_recvParam`/`init_conds` 호출에 `// 1)`·`// 2)` 순번 주석 부여 |

## 2. 화면 개요

유가시장 추가/변경/재상장(신주인수권증권) 상장수수료 계산원칙 안내 팝업. 파라미터 `kk`(3=1건당 10만원, 4=1건당 30만원)에 따라 건당 수수료 문구(`if_kk_3_txt`·`if_kk_4_txt`)를 `$c.util.evalConds` 로 분기 표시하고, [닫기] 버튼은 `$c.win.closePopup` 을 호출한다.
