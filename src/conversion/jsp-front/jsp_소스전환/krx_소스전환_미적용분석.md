...
## 3. 재설계 판단 대상 (자동 치환 불가 — 단계 2 보류 유형) `⏳ 일부 진행 — jQuery 는 규칙 19 전환 완료(잔여 20건 결합 보류)`
...
3. **판단/재설계(단계 2) — 잔여 과제**: §3 유형 — ~~jQuery 94건~~ → **규칙 19 전환 완료(2026-09-07, 잔존 20건은 아래 결합 과제와 함께)**, form 제출 12건(+규칙19-보류 form 기계부 14건), 페이징 DOM 재설계(inf20000), `fn_*` 개명 77건(외부 계약 확인 필요), 컴포넌트 캐싱 전역 22건, layer 2파일 편입 방식 + 각 수정가이드 §3에 기록된 원본 유래 결함(35708 미해결 컴포넌트 참조, 52100 미정의 전역 호출, 20000p 숫자/문자열 비교 등).
