...
3. **판단/재설계(단계 2) — 잔여 과제**: §3 유형 — ~~jQuery 94건~~·~~form 제출 12건~~ → **규칙 19 전환·openFormSubmit 재설계 완료(2026-09-07)**. 잔여: **multipart 파일 전송 제출 4건**(25910 fn_register·52100 3제출·59410 fn_Register — 파일 업로드 API 확정 필요), 페이징 DOM 재설계(inf20000), `fn_*` 개명 77건(외부 계약 확인 필요), 컴포넌트 캐싱 전역 22건, layer 2파일 편입 방식, datepicker 위젯 1건 + 각 수정가이드 §3에 기록된 원본 유래 결함(35708 미해결 컴포넌트 참조, 52100 미정의 전역 호출, 20000p 숫자/문자열 비교 등).
