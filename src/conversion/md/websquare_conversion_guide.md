...
* **멱등성(idempotent)**: 변환본을 다시 돌려도 결과가 동일해야 합니다 — `scwin.vScrenID` 중복 삽입 금지, 이미 `$c.*` 로 바뀐 호출 재치환 금지.
...
## 5. Claude Code 실행용 프롬프트 명령어 (Execution Prompt)

Claude Code CLI 또는 AI Agent 환경에서 해당 규칙을 기반으로 직접 실행할 때 아래 프롬프트를 복사하여 사용할 수 있습니다.