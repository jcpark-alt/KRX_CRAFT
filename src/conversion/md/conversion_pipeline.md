...
| 규칙 21 프레임 접근 → 공통함수 | SCRIPT | `{recv}.Provider("../")`(부모 프레임)를 `$c.win.getParent()` 로 치환(수신 객체 제거). 정확히 `"../"` 리터럴만 | `/top`·`../../`·동적경로(`"../"+x`)·형제프레임은 대응 공통함수 없어 미변환·리포트(단계2). 후속 scwin 멤버는 `.scwin` 경유 검토. 결과에 `.Provider(` 없어 멱등. 상세는 [conversion_rules.md](conversion_rules.md) §규칙 21 |
