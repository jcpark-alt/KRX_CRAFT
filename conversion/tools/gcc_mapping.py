...
단일 출처(SOT): cm/docs/api/{fil,ins,mgt}/index_transfer.html 의 `const DATA = [ ... ];` 배열.
...
def _api_base(base_dir=None):
    """index_transfer.html 들이 위치한 cm/docs/api 경로를 돌려준다."""
    if base_dir is not None:
        return Path(base_dir)
    # 이 파일: conversion/tools/gcc_mapping.py → parents[2] == 저장소 루트
    return Path(__file__).resolve().parents[2] / "cm" / "docs" / "api"
...
def module_fn_dict(base_dir=None):
    ...
    단일 출처(SOT): cm/as-is/{fil,ins,mgt,stf}/gcc/*.xml 각 함수의 JSDoc
    ...
    """
    base = (Path(base_dir) if base_dir else Path(__file__).resolve().parents[2] / "cm") / "as-is"
    cache_key = str(base)
