...
단일 출처(SOT):
  - RAW(파일별 AS-IS→TO-BE 매핑) : cm/docs/api/{fil,ins,mgt}/index_transfer.html 의
...
주입해 `cm/docs/api/gcc/index_transfer.html` 한 파일을 생성한다. 생성물은 손으로 편집하지 않는다.
...
# conversion/tools → 저장소 루트
_REPO = os.path.abspath(os.path.join(_HERE, "..", ".."))
_DEFAULT_OUT = os.path.join(_REPO, "cm", "docs", "api", "gcc", "index_transfer.html")
...
CONV_RULES = [
    ...
    {"asis": '$c.{객체명}.setObjectValue({컴포넌트}, value)',
     ...
     "note": ''},
    {"asis": 'eval({숫자문자열})  /  eval({a} - {b})',
     "tobe": 'Number({숫자문자열})  /  ({a} - {b})',
     "note": '규칙 31. 숫자 변환은 Number() 로 통일, 연산식은 eval 만 제거(괄호 유지)'},
    {"asis": "eval('{id접두}' + i)  /  eval({idVar}).setValue(v)",
     "tobe": "$p.getComponentById('{id접두}' + i)  /  $p.getComponentById({idVar}).setValue(v)",
     "note": '규칙 31. 동적 컴포넌트 참조'},
    {"asis": "eval('(' + {jsonText} + ')')  /  eval('obj.prop.' + {key})",
     "tobe": "JSON.parse({jsonText})  /  obj.prop[{key}]",
     "note": "규칙 31. 코드 문자열 실행(eval('…();'), 정규식 조립 등)은 자동 전환 없이 단계 2 수동 전환"},
]
...
def main(argv=None) -> int:
    ...
    p.add_argument("--base-dir", default=None, help="cm/docs/api 경로(테스트용)")
