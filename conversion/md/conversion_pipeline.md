...
> 기준 파일 예시: `conversion/next-krx-lds-fil-front/ui/ULDCOM00007.xml` — `head(3~430) → script CDATA(22~429) → body(431~568)` 구조.
...
| 규칙 4 영역 재배치 | SCRIPT | 함수 정의를 **5단계 정형화 구조**(2 초기화 = onpageload·onpageunload·`init`/`init_*` / 3 이벤트 / 4 서브미션 콜백 / 5 일반)로 분류·정렬 + 슬래시 섹션 헤더 삽입([code-convention.md](../../cm/docs/code-convention/code-convention.md)). 콜백은 이름 패턴+핸들러 옵션 참조+본문 `$c.sbm.executeDynamic` 호출(통신 실행 함수, 이벤트 핸들러 제외)로 분류. `gform_onload`→`onpageload` 병합은 안전조건에서만 | 함수 사이/뒤 최상위 실행문 있으면 보류·리포트. 구 형식(한 줄 경계 주석·3줄 블록 헤더)은 현행 헤더로 마이그레이션(멱등) |
| 규칙 5 문법/API | SCRIPT | `==`/`!=` → `===`/`!==`, `X.value = v` → `X.setValue(v)`, `X.src = v` → `X.setBackgroundImage(v)`, `X.getTotalRow()` → `X.getRowCount()`, `!X === Y` → `X !== Y`(5e 우선순위 버그 교정·건별 리포트) | 문자열·정규식·주석 리터럴 보호. 속성 대입은 읽기 제외, 메서드명 치환은 수신 객체·인자 보존. 5b/5c 는 body 에 id 가 있는 컴포넌트 수신만 변환(DOM/form 필드는 보류·리포트 — 2026-09-22) |
| 규칙 6 Submission | HEAD+SCRIPT | 정적 action + 단순 `execute(id)` 만, `const sbmOptions = {...}`(submitDoneHandler 제외) + `const sbmRtn = await executeDynamic(sbmOptions)` **async/await 순차 스타일**로 변환 + 노드 삭제. ev:submitdone 핸들러 존재 시 await 뒤 직접 호출 연결. target ID 역추적으로 body `<w2:gridView>` id 를 `gridview` 자동 삽입. 표현식 내 호출은 기존 `await` 유지(이중 await 방지)·대입형은 캡처로 인정(2026-09-22) | 동적 action/속성변형은 sbmOptions 스텁과 함께 리포트. **ev:submiterror 존재 시 콜백 스타일 유지**(단계2 검토) |
...
| 규칙 7n 모듈명 정규화 | SCRIPT | 이미 `$c.<ns>.` 붙은 레거시명 정규화 `$c.stf.fn_setFromToDate(` → `$c.stf.setFromToDate(` (같은 ns, 이름만, 인자 보존) | `gcc_mapping.module_fn_dict()`(SOT: `cm/as-is/*/gcc/*.xml` JSDoc, [substitution_map.md](substitution_map.md) §9) 사용. 네임스페이스 변경/태그 매핑·미정의 함수는 제외(단계 2) |
...
| 규칙 11 include 삭제 | SCRIPT | `include(...)` 로 시작하는 라인 삭제(주석 형태 + **블록 주석 내부 포함** — 2026-09-03 보강, `// #include(...)`·`// //#include(...)` 이중 주석·`#` 변형 포함 — 2026-09-22 보강) | gcc 는 `$c.*` 로 제공되어 불필요 |
| 규칙 30 W-Craft 주석 전부 삭제 | SCRIPT | `//----W-Craft 변환 확인----//` 마커 주석 라인 전부 삭제(블록 주석 내부·`//  //----W-Craft` 이중 주석 변형 포함, 빈 블록 주석 껍데기 동반 제거) + `/* ★Wcraft guide★ … */` 파일 헤더 안내 블록 삭제(2026-09-22 확정, 종전 "유지" 폐기) | guide 블록의 상용구 외 안내 문장(X 변환·eval 사용 수 등)은 단계 2 리포트로 이관. 멱등 |
| 규칙 32 pageList 페이징 공통 전환 | 판단(리포트) | body 에 `<w2:pageList>` 가 있는데 스크립트에 `$c.sbm.setPagingInfo` 가 없으면 `규칙32 … 페이징 공통 미적용` 리포트. 전환은 단계 2 — `scwin.search(gPageNo)` + `scwin.setPaging(res)`(pagingInfo → `$c.sbm.setPagingInfo`) + 화면 자체 pageList/perPage 핸들러·pageList ev:* 속성 삭제 + 참조 0건이 된 page/totalPages 표시 textbox 삭제 (2026-09-23) | 정답지 ULDSTF30702, 상세 [conversion_rules.md](conversion_rules.md) §규칙 32 |
| 규칙 31 `eval` 제거 → 일반 코드 | SCRIPT | `eval(x)`→`Number(x)`, `eval(a - b)`→`(a - b)`, `eval(parseInt(x))`→`parseInt(x)`, `eval("id_" + i)`/`eval(idVar).m()`→`$p.getComponentById(…)`, `eval("(" + json + ")")`→`JSON.parse(json)`, `eval("obj.prop." + k)`→`obj.prop[k]` (2026-09-22 확정) | 문장 단독 실행·상수 코드 문자열·코드 조각 결합·정규식 조립·DOM 속성 접근(`.innerHTML` 등)은 보류·리포트(단계 2). 리터럴 보호, `window.eval` 대상 외, 멱등. 상세는 [conversion_rules.md](conversion_rules.md) §규칙 31 |
...
| 규칙 27 그리드 자식 중복 id 재부여 | BODY(XML) | `<w2:caption>`/`<w2:header>`/`<w2:gBody>`/`<w2:row>`(row 는 2026-09-22 추가) 의 문서 전체 중복 id 를 `{base}{n}` 미사용 순번으로 재부여(WS120 해소 — W-Craft 가 그리드마다 caption1 등 복제) | 첫 등장 유지·표시 전용 id 만 대상(스크립트 참조 없음). 상세는 [conversion_rules.md](conversion_rules.md) §규칙 27 |
...
* **browserPopup 부모 접근 보강**: browserPopup 으로 열리는 팝업 화면의 `window.opener.*`·부모 scwin 호출은 `$c.win.getOpenerScope()`/`$c.win.callOpener()` 로 재작성한다(`getParent()` 불가 — `cm/docs/popup-opener-guide.md`).
* **목록↔상세 복귀 상태 복원**: 목록→상세 이동(moveUrl/setPageFrameSrc) 화면은 `{isHistory:true, dataInfo}` 스냅샷과 상세의 [목록] 버튼 `{restoreData:true}` 패턴을 적용해 조회조건·목록·페이징을 복원한다(`cm/docs/frame-history-guide.md`, `_pagingInfo` 예약 키).
