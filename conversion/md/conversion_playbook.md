...
- [ ] **gcc 공통 라이브러리 배포** — 대상 프로젝트 WebSquare 서버에 `cm/gcc/` 11개 파일(`$c.util/win/str/num/date/data/validate/sbm/hkey/session/ext`)이 배포되어 `$c.*` 로 접근 가능
...
- [ ] **모듈별 업무공통(pcc) 트리 고정** — 전환 대상 모듈에 따라 참조할 업무공통이 정해져 있다(2026-09-21 확정): `conversion/jsp-front/**`·`conversion/next-krx-lds-fil-front/**` 화면은 **`cm/pcc/fil/`**(`$c.fil`·`$c.cm`·`$c.cp`·`$c.dis`), `conversion/next-krx-lds-stf-front/**` 화면은 **`cm/pcc/stf/`**(`$c.stf`·`$c.lc`·`$c.cm`·`$c.bns`·`$c.print`·`$c.cp`), `conversion/next-krx-lds-mgt-front/**` 화면은 **`cm/pcc/mgt/`**(`$c.mgt`), `conversion/next-krx-lds-tms-front/**` 화면은 **`cm/pcc/tms/`**(`$c.tms`). 각 트리는 같은 네임스페이스(`$c.cm`·`$c.cp` 등)를 따로 정의하고 배포 프로젝트마다 한 트리만 로드된다 — 다른 모듈 트리의 함수(예: mgt 화면이 pcc/stf 의 `$c.cm`·`$c.cp`·`$c.stf` 를 부르는 것)는 실환경 미정의 호출이 되며, 필요한 함수는 해당 모듈 pcc 트리에 반입하거나 화면 로컬로 둔다. 미정의 `$c` 함수 대조·레거시 치환 시 publicInfo 대조 대상을 해당 트리로 한정한다(gcc `cm/gcc` 는 공통).

## 1. 작업 공간 구성

프로젝트별 폴더를 `conversion/` 아래 표준 규격으로 만든다.

```
conversion/<프로젝트명>-front/
...
python conversion/tools/convert_all.py <프로젝트명>-front

# 규칙 개정 후 전체 재생성 (기존 산출물 덮어씀)
python conversion/tools/convert_all.py --force <프로젝트명>-front
```
...
- [ ] `python -m pytest conversion/tools/` — 변환기 회귀 테스트 통과 (규칙 개정을 했다면 필수)
