...
* **초기화 함수 분류**(2026-09-23): `scwin.init` / `scwin.init_*` 는 2구역(초기화 영역)으로 분류해 `onpageload`(→ `onpageunload`) 다음에
  배치한다(code-convention '초기화' 절 — onpageload 는 init_* 를 순차 호출하고 로딩 로직을 직접 갖지 않는다). 종전에는 통신 없는 일반 함수로 5구역에 놓였다.
  sample-front/ui/fil·stf 35화면 소급 적용(onpageload 인라인 초기화 → init 분리 23화면).
...
* **async/await 순차 실행 우선**([code-convention.md](../../cm/docs/code-convention/code-convention.md)): 스크립트의 `$c.sbm.execute(sbm_x);` 를
...
* **단일 출처(SOT)**: 원본 함수 → 대상 함수 매핑의 기준은 모듈별 이관 매핑 가이드인 `cm/docs/api/{fil,ins,mgt}/index_transfer.html` 의 `DATA` 배열입니다. 변환 대상 파일이 속한 모듈의 매핑을 우선 적용하고, 대표 매핑은 별도 문서 [substitution_map.md](substitution_map.md) 에 정리되어 있습니다.
* **태그(tag) 의미**:
    * *(태그 없음)*: 1:1 직접 치환 가능.
...
* **치환 시 유의사항**:
    * 인자 순서·개수가 레거시와 다를 수 있으므로, 치환 전 gcc 함수 시그니처(`cm/docs/api/gcc/index.html`)를 반드시 확인합니다.
...
* **단일 출처(SOT)**: 규칙 7(레거시 유틸 → `$c.core`, `index_transfer.html`)과 달리, 이 매핑은 **모듈 업무공통 함수**(substitution_map.md **§9**)이며 `cm/as-is/{fil,ins,mgt,stf}/gcc/*.xml` 각 함수의 JSDoc `(AS-IS: 원본명)` ↔ `@name`/`scwin.<정규명>` 정의에서 직접 취합합니다(`gcc_mapping.module_fn_dict()`).
* **치환 시 유의사항**:
...
* **삭제 범위**: 활성 코드뿐 아니라 주석 처리된 형태(`//include(...)`, `////include(...)`)와
  **블록 주석(`/* */`)으로 감싼 include 묶음**도 제거합니다(2026-09-03 보강 — `ULDSTF30304` 잔존 사례 대응).
  W-Craft 가 남기는 **이중 주석 변형**(`// #include("../js/session.js");`, `// //#include("../js/PopupCalendar.js");` 처럼
  주석 접두가 여러 겹이거나 `#` 가 붙은 형태)도 삭제합니다(2026-09-22 보강 — `ULDSTF07400` 계열·`ULDMGT50002` 잔존 사례 대응).
...
* **변환 규칙**:
    ...
    4. `"browserPopup"` 이면 **`options.callbackFn: "scwin.popupCallback"`** 지정 + `scwin.popupCallback` 정의를 파일에 1회 추가합니다(**await/result 미사용** — 콜백으로 비동기 수신).
    5. `CreateDialogFrame` 바로 윗줄이 인자에 `row` 를 넘기는 함수 호출(예: `fn_setId(row);`)이면 삭제합니다.
* **팝업 타입별 데이터 수신 규약(2026-09-01 확정)**: 리턴 데이터는 `options.type` 별 정의된 방식으로만 수신합니다.
    * `"pageFramePopup"` → **async/await 동기 수신**: `const result = await $c.win.openPopup(url, options, data);`
    * `"browserPopup"` → **`options.callbackFn` 콜백 비동기 수신**: `$c.win.openPopup(url, options);` (`callbackFn : "scwin.popupCallback"` 지정, await 미사용)
    * gcc `openPopup` 이 `options.callbackFn` 을 내부 채널(`data.callbackFn`)로 브리지합니다(기존 data 전달 하위호환, options 우선). `data` 페이로드는 pageFramePopup 전용.
...
* **browserPopup 자식 화면의 부모 접근**: `"browserPopup"` 은 별도 브라우저 창이라 자식 화면의 부모 접근 코드(`window.opener.*`, 레거시 `Provider("../")` 후 scwin 호출 등)는 `$c.win.getParent()` 로 변환하면 **동작하지 않습니다**. 팝업 타입 무관 공통함수인 `$c.win.getOpenerScope()`(부모 scope)·`$c.win.callOpener(fnName, ...args)`(부모 scwin 함수 호출)로 재작성합니다 — **단계 2 검토 보강 대상**. 상세는 `cm/docs/popup-opener-guide.md` 참조.
...
* **치환 시 유의사항**:
    ...
    * 인자는 그대로 보존하고 네임스페이스(및 `getSysDate` 의 경우 함수명)만 교체합니다. 시그니처 차이가 의심되면(예: 쿠키 `options`·웹스토리지 직렬화 동작) gcc 함수 시그니처(`cm/gcc/util.xml`·`cm/gcc/date.xml`)를 확인한 뒤 치환합니다.
...
* **browserPopup 예외**: 변환 대상 화면이 **browserPopup 으로 열리는 팝업 화면**(규칙 17 `type:"window"` 계열)이면 `$c.win.getParent()` 는 부모 화면이 아니라 자기 자신을 가리켜 동작하지 않습니다. 이 경우 `$c.win.getOpenerScope()`/`$c.win.callOpener()` 로 치환합니다(팝업 타입 무관 동작 — pageFramePopup 은 내부에서 `$p.parent()` 폴백). **단계 2 검토** 대상이며, 상세는 `cm/docs/popup-opener-guide.md` 참조.
...
### 규칙 30: W-Craft 주석 전부 삭제 (변환 확인 마커 + `★Wcraft guide★` 안내 블록)
...
* **삭제 범위**: 블록 주석(`/* */`) 내부의 마커 라인 포함. **이중 주석 변형**(`//     //----W-Craft …----//`,
  `// \t//----W-Craft …----//` 처럼 주석 접두가 여러 겹인 라인, 끝 `----/` 결손 포함)도 삭제합니다(2026-09-22 보강).
  마커·include(규칙 11) 삭제로 내용이 비게 된 블록 주석 껍데기(`/*` ~ `*/` 사이가 공백뿐)도 함께 제거합니다.
* **`★Wcraft guide★` 안내 블록 삭제**(2026-09-22 확정 — 종전 "유지" 정책 폐기): 파일 헤더의
  `/* ★Wcraft guide★ … 스크립트 수작업 유의사항 … */` 블록도 W-Craft 도구 주석이므로 삭제합니다.
  블록 안에 상용구(제목·구분선·"N. include 제거") 외의 안내 문장이 있으면(예: "ASIS에서 문법에 어긋난 문장 … X 로 변환",
  "eval함수 사용 수: 2") 삭제 전에 `규칙30 ★Wcraft guide★ 안내 삭제 — 단계2 확인 사항: …` 으로 **리포트에 이관**해
  단계 2에서 코드 내 `X` 토큰·eval 잔존(규칙 31) 등을 확인할 수 있게 합니다.
* convert.py 포매팅 단계(`remove_wcraft_markers`)가 자동 적용하며 멱등입니다. 규칙 7m/12/20 이 개별 치환 시
  직전 마커를 함께 지우는 동작은 그대로 유지됩니다(본 규칙이 최종 일괄 정리).
* 참고: `cm/as-is/**` 의 레거시 원본에 남은 guide 블록은 변환 산출물이 아니므로 본 규칙의 대상이 아닙니다.
