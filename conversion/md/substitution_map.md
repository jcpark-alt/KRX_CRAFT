...
> **프로그램 접근**: 이 표는 사람이 읽는 요약이고, 변환기는 `conversion/tools/gcc_mapping.py` 로더로 위 `DATA`(SOT)를 직접 파싱해 사용합니다. `substitution_dict()` 는 **태그 없는 순수 식별자·무충돌** 항목만 담은 자동 1:1 치환 사전(규칙 7)을, `conflicts()` 는 동일 이름이 다른 `$c.*` 로 갈리는 충돌 항목을 돌려줍니다. (`python conversion/tools/gcc_mapping.py` 로 요약 확인)
...
이 표는 위 1~8장(레거시 유틸 → `$c.<core>` 대표 치환)과 달리, **모듈별 업무공통(KRX 고유) 함수의 AS-IS 레거시 원본 → 변환된 `$c.<모듈>` 함수 1:1 매핑**이다. `cm/as-is/{fil,ins,mgt,stf}/gcc/*.xml` 각 함수 JSDoc 의 `(AS-IS: 원본명, origin: 원본.xml)` 주석을 취합한 것으로, 변환 추적성(traceability) 규칙은 [stf/md/CONVERSION.md](../../cm/as-is/stf/md/CONVERSION.md) 를 따른다.
