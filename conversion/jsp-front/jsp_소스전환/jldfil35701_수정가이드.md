# jldfil35701 수정 가이드 — editor-web generate 산출물 후속 정비

> 원본: `../krx_소스전환/jldfil35701.xml` → 정비본: `jsp-front/jldfil35701.xml` (2026-09-07)
...
| 과밀 라인 분해 | 1건 | init_conds 의 binds 배열 한 줄(약 430자) → 바인딩별 다중행 객체 리터럴 |
| onpageload 재배치 | O | 2구역 최상단으로 이동(init_conds·init_recvParam 앞), init_* 호출에 `// 1)`·`// 2)` 순번 주석 부여 |

## 2. 화면 개요

유가증권시장 신규상장 수수료 계산원칙 안내 팝업. 전환 파라미터 `kk`(1: 보통주권/외국주권/종류주권/기업인수목적회사주권, 2: 집합투자증권)에 따라 제목(txt_h17/txt_h19)과 하단 안내 영역(if_kk_3/if_kk_4)의 표시를 전환하고, 시가총액 구간별 수수료율 정적 표를 보여준 뒤 [닫기](btn_self)로 팝업을 닫는다.
...
> `convert.py` 단계1을 제자리 실행하고 후처리로 보정(init 2구역 복원·5b DOM 수신 원복·개명 충돌 조정·JSDoc 동기화). 경위·공통 게이트: [krx_소스전환_미적용분석.md §5 이력 6](krx_소스전환_미적용분석.md). diff +2/−16.
...
| `$c.data.readValue(dc, key, opts)` | 4 | dataMap → `dc.get("key")` 4건, dataList → `dc.getCellData(row, "key")` 0건, 변수 키 래퍼 → `.get(key)` 0건. `silent`/`soft`/`ctx`/`label` 옵션은 의미 없어 제거 |
