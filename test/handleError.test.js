...
function loadHarness() {
  ...
  sandbox.$c.exception = sandbox.scwin;   // 실환경 네임스페이스 배선
  const origReport = sandbox.scwin._reportError;
  sandbox.scwin._reportError = (ex, context) => { calls.report.push({ ex, context }); return origReport(ex, context); };
...
describe("handleError 공통 오류 처리 (src/gcc/exception.xml)", () => {
  ...
  test("reportError 가 예외를 던져도 사용자 알림은 정상 수행", async () => {
    h.sandbox.scwin._reportError = () => { throw new Error("report down"); };
